...
3. `bxcodegen:generate module -o name=vendor.module_name` - создать код с использованием кастомного генератора указанного в первом параметре, все настройки передаются с помощью ключа -o, в значении которого лежит пара "имя опции=значение опции", разделенная символом `=`.
